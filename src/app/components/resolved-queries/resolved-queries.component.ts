...
@Component({
  selector: 'app-resolved-queries',
  templateUrl: './resolved-queries.component.html',
  styleUrls: ['./resolved-queries.component.css']
})
export class ResolvedQueriesComponent implements OnInit {
  show :boolean =false;
  rowNumber:number =-1;
  loading:boolean=false;
  allRequestDetials:any=[];
  isAdmin:boolean=false;
  constructor(private queryService:QueryService,private toastr:ToastrManager,
  ...
  onShowMoreClicked(index:number){
   ...
   get_top_queries_details(){
       ...
       this.queryService.get_top_10_fixed_queries(localStorage.getItem('email')).pipe(
            result=>{
                     return result;
                    },
            catchError(
