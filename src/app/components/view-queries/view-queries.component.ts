...
export class ViewQueriesComponent implements OnInit {
  ...
  get_all_pending_request_details(){
    ...
    let loginEmail=localStorage.getItem('email'); 
    if(loginEmail==='pema@maxonic.com' ||loginEmail==='nishant@maxonic.com'||loginEmail==='paduka@maxonic.com' ){
       this.queryService.get_request_details_for_admin(localStorage.getItem('email')).pipe(
            result=>{
                     return result;
                    },
            catchError(
            (err)=>{
                 if(err ==="No pending request found in database"){
       ...
       this.queryService.get_request_details(localStorage.getItem('email')).subscribe((result)=>{
     console.log(result);
     if(result["result"].length === 0){
      this.allRequestDetials=[]
